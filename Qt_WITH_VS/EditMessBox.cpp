...
EditMessBox::EditMessBox()
{
    ...
    */
    ...
    if (checkList == 1)
        ...
        FormLayout->addRow("留校原因：", qvisReason);
...
void EditMessBox::saveMess() {
    ...
    User* us;
    //根据类型选择不同指向，并且判断人数是否已满
    switch (checkList)
    {
    case 1:
        if (stucount >= SIZE)
        {
            QMessageBox::warning(this, tr("提示"), tr("学生人数已满！"), QMessageBox::Yes);
            return;
        }
    ...
    case 2:
        if (teacount >= SIZE)
        {
            QMessageBox::warning(this, tr("提示"), tr("教职工人数已满！"), QMessageBox::Yes);
            return;
        }
    ...
    default:
        if (viscount >= SIZE)
        {
            QMessageBox::warning(this, tr("提示"), tr("访客人数已满！"), QMessageBox::Yes);
            return;
        }
        us = new VISITOR_User;
        break;
    ...
    else if (this->qclass->currentText() != "学生" && this->qcode->currentText() == "红码" && this->qbuilding->text() != "大学城中心酒店")
    {
        int click = QMessageBox::warning(this, tr("提示"),
            tr("红码教职工和访客需要入住大学城中心酒店\n选择“是”将确定保存，系统自动修改为中心酒店和隔离状态\n"
                "选择“否”将返回填写页面"),
            QMessageBox::Yes, QMessageBox::No);
        if (click == QMessageBox::Yes)
        {
    ...
    //判断证件号是否存在
    if (us->IdIsExisted(this->qid->text()))
    {
        QMessageBox::warning(this, tr("提示：未能成功注册"),
            tr("证件号重复"),
            QMessageBox::Yes);
