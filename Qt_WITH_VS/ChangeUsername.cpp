...
void ChangeUsername::on_OK_clicked()
{
	...
	if (type == 2)//学生
	{
		for (int i = 0;i < SIZE && !StudentUser[i].getUsername().isEmpty(); ++i)//数组内查重
		{
			if (qusername == StudentUser[i].getUsername())
			{
				if(i != Pos)
	...
	{
		for (int i = 0; i < SIZE && !VisitorUser[i].getUsername().isEmpty(); ++i)//数组内查重
		{
			if (qusername == VisitorUser[i].getUsername())
			{
				if (i != Pos)
					QMessageBox::warning(this, tr("提示"), tr("用户名已被使用\n请更换另一个用户名"), QMessageBox::Ok);
				else
